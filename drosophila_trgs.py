...
# inputs
species_filename = dict([('DROME', 'GCF_000001215.4_Release_6_plus_ISO1_MT'), ('DROSI', 'GCF_000754195.2_ASM75419v2'), (
    'DROSE', 'GCF_000005215.3_dsec_caf1'), ('DROYA', 'GCF_000005975.2_dyak_caf1'), ('DROER', 'GCF_000005135.1_dere_caf1')])
species_sciname = dict([('DROME', 'Drosophila melanogaster'), ('DROSI', 'Drosophila simulans'), (
    'DROSE', 'Drosophila sechellia'), ('DROYA', 'Drosophila yakuba'), ('DROER', 'Drosophila erecta')])
clade = ['DROSI', 'DROSE', 'DROME']
clade_sci = ['Drosophila simulans',
...
#  Run OMA - this happens elsewhere.
...
for hog in tqdm(hogs):
    ...
    if len(outgroup_members) == 0:
        ...
        faa = ''
        for record in records:
            faa += '>%s [%s]\n%s\n' % (oma_to_refseq[record.id],
                                       record.id[:5], str(record.seq))
...
for hog in tqdm(putative_trg_hogs1, miniters=0, mininterval=1, maxinterval=5):
    ...
    if not os.path.isfile('%s_vs_oma.blastp' % (hog)):
        cmd = "blastp -db %s -query %s.faa -out %s_vs_oma.blastp -num_threads %s -evalue 1 -max_target_seqs 500 -word_size 3 -gapopen 11 -gapextend 1 -matrix BLOSUM62 -threshold 11 -comp_based_stats 2 -seg no -soft_masking false -xdrop_ungap 20 -xdrop_gap 30 -xdrop_gap_final 25 -window_size 40 -outfmt '%s'" % (
            aa_blast_db, hog, hog, cpu, blast_out_format)
        subprocess.run(cmd, shell=True)
...
for hog in non_trgs:
    cmd = 'mv %s %s/putative_trg_families_with_blastp_hits_in_oma' % (
        hog, results_dir)
    subprocess.run(cmd, shell=True)
            ...
            if not line.startswith('#'):
                ...
                if this_line[4][:5] not in clade:
                    ...
                    if float(this_line[10])/int(this_line[16]) > lencutoff:
                        # if evalue is < 0.001
                        if float(this_line[3]) < ecutoff:
                            gene_hits[this_line[0]].append(
                                this_line[4].split('|')[-1])
    for k, v in gene_hits.items():
        cmd = 'efetch -format docsum -db protein -id %s | xtract -pattern DocumentSummary -element Title' % (
            ','.join(list(set(v))))
        species = subprocess.check_output(
            cmd, stderr=subprocess.STDOUT, shell=True)
        species = species.decode('utf-8').split('\n')
        species = [x.split('[')[-1].split(']')[0]
                   for x in species if len(x) > 0]
...
trg_genes = pandas.DataFrame(
    columns=['HOG', 'Species', 'Protein', 'Gene', 'fna'])
# add genes from TRG HOGs to df
for hog in putative_trg_hogs3:
    records = list(SeqIO.parse('%s/%s/%s.faa' %
                               (results_dir, hog, hog), 'fasta'))
    for record in records:
        trg_genes.loc[len(trg_genes)] = [
            hog, record.description.split('[')[1][:-1], record.id, '', '']
...
for key, value in species_filename.items():
    print(key)
    genome_fasta = SeqIO.index('%s/%s_genomic.fna' %
                               (genomes_dir, value), 'fasta')
    db = gffutils.FeatureDB('%s/%s_genomic.gffdb' %
                            (genomes_dir, value), keep_order=True)
    ...
    for index, row in tqdm(trg_genes.iterrows()):
        ...
        if trg_genes.iloc[index]['Species'] == key:
            ...
            cds_of_this_protein = [
                x for x in cds_list if x['protein_id'][0] == protein]
            ...
            if len(set(gene)) == 1:
                ...
                cds_of_this_gene = list(db.children(
                    gene[0], featuretype='CDS', order_by='start'))
                ...
                fna = '>%s [%s] %s:%s-%s \n%s\n' % (protein, key, chromosome, gene[0].start, gene[0].end, str(
                    genome_fasta[chromosome].seq)[gene[0].start-1:gene[0].end])
                cds_coordinates = []
                for cds in cds_of_this_gene:
                    ...
                    if coords not in cds_coordinates:
                        ...
                        fna += '>%s_cds_%s [%s] %s:%s-%s \n%s\n' % (protein, len(
                            cds_coordinates), key, chromosome, cds.start, cds.end, str(genome_fasta[chromosome].seq)[cds.start-1:cds.end])
...
for hog in tqdm(putative_trg_hogs3):
    os.chdir("%s/%s" % (results_dir, hog))
    for k, v in species_filename.items():
        cmd = "blastn -db %s/%s_genomic.fna -query %s.fna -out %s_vs_%s.blastn -num_threads %s -evalue 1 -word_size 7 -max_target_seqs 500 -gapopen 5 -gapextend 2 -reward 2 -penalty -3 -strand both -perc_identity 0 -outfmt '%s'" % (
            genomes_dir, v, hog, hog, k, cpu, blast_out_format)
        subprocess.run(cmd, shell=True)
                ...
                if not line.startswith('#'):
                    ...
                    if float(tl[3]) <= ecutoff:
                        ...
                        if float(this_line[10])/int(this_line[16]) > lencutoff:
                            ...
                            trg_hits.setdefault('%s\t%s\t%s' % (hog, k, tl[4]), []).append(
                                [int(start), int(end), float(tl[3])])

# create df with genome regions
trg_df = pandas.DataFrame(
    columns=['HOG', 'Species', 'Chromosome', 'Start', 'End', 'E-value', 'Sequence'])
for k, v in tqdm(trg_hits.items()):
    ...
    for i in range(1, len(locations)):
        ...
        if locations[i][0] < current_location[1]:
            current_location = [current_location[0], max(
                current_location[1], locations[i][1]), min(current_location[2], locations[i][2])]
        else:
    ...
    for i in range(len(locations_unique)):
        trg_df.loc[len(trg_df)] = [k.split()[0], k.split()[1], k.split()[
            2], locations_unique[i][0], locations_unique[i][1], locations_unique[i][2], '']
...
genome_fasta = {}
for k, v in species_filename.items():
    genome_fasta[k] = SeqIO.index(
        '%s/%s_genomic.fna' % (genomes_dir, v), 'fasta')
...
for hog in putative_trg_hogs3:
    ...
    for index, row in tqdm(trg_df.iterrows()):
        if trg_df.iloc[index]['HOG'] == hog:
            species_chromosome_fna.setdefault('%s-%s' % (trg_df.iloc[index]['Species'], trg_df.iloc[index]['Chromosome']), []).append(
...
for hog in ['HOG13860', 'HOG13765', 'HOG13746', 'HOG13754', 'HOG13766', 'HOG13767', 'HOG14054']:
    ...
    for species in ordered_species:
        ...
        for index, row in sub_hog_df.iterrows():
            fna += '>%s-%s-%s:%s\n%s\n' % (trg_df.iloc[index]['Species'], trg_df.iloc[index]['Chromosome'],
                                           trg_df.iloc[index]['Start'], trg_df.iloc[index]['End'], trg_df.iloc[index]['Sequence'])
...
extra_outgroup_species_filename = dict([('DROAN', 'GCF_000005115.1_dana_caf1'), (
    'DROSU', 'GCF_000472105.1_Dsuzukii.v01'), ('DROPS', 'GCF_000001765.3_Dpse_3.0'), ('DROMI', 'GCF_000269505.1_DroMir_2.2')])
                ...
                if not line.startswith('#'):
                    ...
                    if float(tl[3]) <= ecutoff:
                        ...
                        if float(tl[10])/int(tl[16]) > lencutoff:
                            ...
                            trg_hits_2.setdefault('%s\t%s\t%s' % (hog, k, tl[4]), []).append(
                                [int(start), int(end), float(tl[3])])
...
for k, v in species_filename.items():
    db = gffutils.FeatureDB('%s/%s_genomic.gffdb' %
                            (genomes_dir, v), keep_order=True)
    scaffold = 'NT_167065.1'
    start = 1351813
    end = 1352284
    annotations = list(
        db.region(region=(scaffold, start, end), completely_within=False))
    ...
    for line in f.readlines():
        if line.startswith('# Query: '):
            ...
            if hit_name not in added_hits:
                ...
                element_hits.setdefault(tl[4], []).append(
                    [element_name, min([int(tl[5]), int(tl[6])]), max([int(tl[5]), int(tl[6])])])

genome_dna_seqs = SeqIO.index(
    '%s/%s' % (trgf_hog_dir, genome_dna_seqs_file), 'fasta')
element_dna_seqs = SeqIO.index(
    '%s/%s' % (trgf_hog_dir, element_dna_seqs_file), 'fasta')
...
for k, v in element_hits.items():
    ...
    for i in range(1, len(hits)):
        if hits[i][1] < end:
            ...
            fasta = '>%s-%s-%s:%s\n%s\n' % (k.split('-')[0], k.split('-')[1], str(
                start), str(end), str(genome_dna_seqs[k].seq)[start-1:end])
            with open('%s/%s.fna' % (trgf_hog_dir, file_name), 'w') as f:
                f.write(fasta)
            fasta = ''
            for element in element_list:
                fasta += '>%s\n%s\n' % (element,
                                        str(element_dna_seqs[element.split()[0]].seq))
...
for file_name in file_name_list:
    cmd = "mafft --genafpair --maxiterate 1000 --thread 3 --adjustdirectionaccurately --multipair --addfragments %s_elements.fna %s.fna > %s.msa" % (
